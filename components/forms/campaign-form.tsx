...
export function CampaignForm({ campaignId }: CampaignFormProps) {
  ...
  const [name, setName] = useState(isEditMode ? "" : "Untitled Campaign")
  ...
  // Load campaign if in edit mode
  useEffect(() => {
    if (!isEditMode || hasLoadedCampaignRef.current) return
  ...
  const loadBriefings = async () => {
    try {
      ...
      // Set first briefing as default if available AND not edit mode
      if (!isEditMode && response.items.length > 0 && !briefings[0].id) {
  ...
  const loadCampaign = async () => {
    ...
    try {
      ...
      const campaign = await apiClient.campaigns.get(campaignId)
      
      setName(campaign.name)
      setDescription(campaign.description || "")
      setRecipientInput(campaign.recipient_emails.join(", "))
      
      if (campaign.briefing_ids && campaign.briefing_ids.length > 0) {
        setBriefings(campaign.briefing_ids.map(id => ({ id })))
      }
      
      // Parse schedule description
      if (campaign.schedule_description && campaign.schedule_description !== "Not scheduled") {
        if (campaign.schedule_description.startsWith("Daily at ")) {
          const timesStr = campaign.schedule_description.substring(9)
          const parsedTimes = timesStr.split(",").map(t => ({
            id: generateId(),
            time: t.trim()
          }))
          setTimes(parsedTimes)
        }
        // Add other parsing logic here if needed
      } else {
        setTimes([]) // Or keep default
      }
  ...
  const validate = () => {
    ...
    // Times are optional if "Not scheduled" is allowed, but UI enforces it currently.
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Join them carefully. If multiple times, we can join with comma? 
      // For now, the backend parses "Daily at 09:00, 17:00" BUT our new format is different per row.
      // Actually, we probably want to support MULTIPLE different schedules combined?
      // The current backend parser handles a single string description.
      // It splits by comma if it detects multiple times in "Daily at...".
      // BUT if we have "Daily at 09:00" AND "Weekly on Mon at 10:00", simply joining with comma might be ambiguous:
      // "Daily at 09:00, Weekly on Mon at 10:00" -> Backend parser might need update to split these distinct schedules properly.
      // Let's check backend parser again.
      // It takes `schedule_description` string.
      // It checks `if "weekly on" in schedule.lower():` -> single block.
      // It checks `if "daily" in schedule.lower()` -> single block.
      // It doesn't seem to support MIXED types in one string easily without more work.
      
      // FOR NOW, let's assume the user picks ONE type of schedule or we join them with a delimiter the backend ignores/handles?
      // Actually, the UI allows adding multiple rows. 
      // If I add "Daily at 09:00" and "Weekly on Mon at 10:00", I need to send both.
      // Let's send them as a newline separated string? Or generic separator?
      // The backend parser is: `schedule = schedule_description.strip()`.
      // It doesn't seem to loop over lines.
      
      // To support multiple complex schedules, we should probably update the backend to split by some delimiter (like " & " or newline) first.
      // For MVP, let's just join with " & " and update the backend parser to split by " & " at the top level.
      
      const scheduleDesc = times.length > 0 
        ? times.map(t => t.time).filter(Boolean).join(" & ")
        : "Not scheduled"
      ...
      if (isEditMode && campaignId) {
        await apiClient.campaigns.update(campaignId, payload)
        toast({
    ...
    <div className="flex-1 flex flex-col p-6 gap-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-semibold">{isEditMode ? "Edit Campaign" : "Create Campaign"}</h1>
          <p className="text-muted-foreground text-sm">
            {isEditMode ? "Update your campaign settings, recipients, and schedule." : "Bundle your saved briefings, define recipients, and choose delivery moments."}
          </p>
        </div>
